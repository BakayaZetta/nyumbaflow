...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['log_visitor'])) {
    ...
    $stmt = $pdo->prepare("INSERT INTO visitors (property_id, tenant_id, name, id_number, phone_number, number_plate, id_image, visitor_type, purpose, visit_date, time_in) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([$property['id'], $data['tenant_id'] ?? null, $name, $id_number, $phone, $plate, $id_image, $v_type, $v_purpose, $v_date, $v_time]);
