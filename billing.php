...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_readings'])) {
    ...
    header("Location: billing.php?property_id=" . (int)($_POST['property_id'] ?? 0) . "&tab=readings");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_invoice'])) {
    ...
    header("Location: billing.php?property_id=" . (int)($_POST['property_id'] ?? 0));
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_adjustment'])) {
    $tenant_id = intval($_POST['adjust_tenant_id'] ?? 0);
    $adj_month = $_POST['adjust_month'] ?? '';
    $adj_year = $_POST['adjust_year'] ?? '';
    $adjustment_type = $_POST['adjustment_type'] ?? '';
    $amount = floatval($_POST['adjustment_amount'] ?? 0);
            ...
            <div class="filter-bar">
                ...
                <select class="prop-select" onchange="location.href='?property_id='+this.value">
                    <?php foreach ($properties as $p): ?>
                        <option value="<?php echo esc($p['id']); ?>" <?php echo $p['id'] == $current_property_id ? 'selected' : ''; ?>>
                            <?php echo esc($p['name']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
