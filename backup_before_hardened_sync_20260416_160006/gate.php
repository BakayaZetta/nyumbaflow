...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['log_visitor'])) {
    ...
    if (isset($_FILES['id_image']) && $_FILES['id_image']['error'] === 0) {
        ...
        $upload_path = 'uploads/visitors/' . $id_image;
        move_uploaded_file($_FILES['id_image']['tmp_name'], $upload_path);
    }
    ...
    // UPDATED: Include id_image in the INSERT statement
    $stmt = $pdo->prepare("INSERT INTO visitors (property_id, tenant_id, name, id_number, phone_number, number_plate, id_image, visit_date, time_in) VALUES (?, ?, ?, ?, ?, ?, ?, CURDATE(), CURTIME())");
    $stmt->execute([$property['id'], $data['tenant_id'] ?? null, $name, $id_number, $phone, $plate, $id_image]);
    ...
    if ($data) {
        ...
        $vMsg = "Hi $name, you have visited $propName to unit $unitNum, visiting $tenantName.";
...
<head>
    ...
    <title>Security Portal - <?php echo htmlspecialchars($property['name']); ?></title>
    ...
    <style>
        ...
        .visitor-photo { width: 40px; height: 40px; border-radius: 50%; object-fit: cover; margin-right: 10px; border: 2px solid var(--primary); }
        ...
        <div class="card">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <small style="color: #64748b; font-size: 11px;">Photo will be saved and linked to this visitor</small>
                </div>
        ...
        <div style="margin-top:40px;">
            ...
            <div id="activeList">
                <?php foreach ($active_visitors as $v): ?>
                    <div class="visitor-item" data-search="<?php echo strtolower($v['name'] . ' ' . ($v['unit_number'] ?? 'No Unit')); ?>">
                        <div class="visitor-info">
                            <?php if (!empty($v['id_image']) && file_exists('uploads/visitors/' . $v['id_image'])): ?>
                                <img src="uploads/visitors/<?php echo htmlspecialchars($v['id_image']); ?>" class="visitor-photo" alt="Visitor photo">
                            <?php else: ?>
                                <div style="width:40px; height:40px; border-radius:50%; background:#e2e8f0; display:flex; align-items:center; justify-content:center; margin-right:10px;">
                            ...
                            <div>
                                ...
                                <div style="display:flex; gap:10px; margin-top:5px;">
                                    <span class="badge badge-info">Unit <?php echo htmlspecialchars($v['unit_number'] ?? 'N/A'); ?></span>
